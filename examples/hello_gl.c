...
struct {
	...
	bool fullscreen;
} window = { L"OpenGL", "OpenGL", L"OpenGL Example", NULL, NULL, NULL, 0, 0, 0, 0, 0, 0, 640, 480, false };
...
createWindow(HINSTANCE instance)
{
	if (err.code == ERR_NONE)
	{
		...
		if (AdjustWindowRect(&rect, style, false))
		{
			...
			window.hndl = CreateWindowW(window.className, window.title, style, 0, 0, window.width, window.height, NULL, NULL, instance, NULL);
			if (!window.hndl)
			{
				err.code = ERR_CRWIN;
        ...
        )
{
	...
	{
		wchar_t *title = (wchar_t*)malloc(10*sizeof(wchar_t));
